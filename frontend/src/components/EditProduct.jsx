...
const EditProduct = ({ data: productData, close, fetchData }) => {
  ...
  return (
    <section className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-50 px-4">
          ...
          {["unit", "stock", "price", "discount"].map((field) => (
            <div className="grid gap-1" key={field}>
              ...
              <input
                ...
                className="w-full p-2 border border-blue-200 outline-none bg-blue-50 rounded"
                onKeyDown={(e) => {
                  if (["stock", "price", "discount"].includes(field) && ["e", "E", "+", "-"].includes(e.key)) {
                    e.preventDefault();
                  }
                }}
              />
