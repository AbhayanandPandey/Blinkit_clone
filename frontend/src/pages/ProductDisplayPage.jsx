...
const ProductDisplayPage = () => {
  ...
  const [showImage, setShowImage] = useState(false) // ✅ state for modal
  ...
  return (
    <>
        ...
        <>
          <section className='bg-white mx-auto lg:px-8 md:px-8 p-4 px-4 grid lg:grid-cols-2 '>
            <div className='col-span-1'>
              <div
                className='bg-white lg:min-h-[60vh] lg:max-h-[60vh] md:min-h-[50vh] md:max-h-[50vh] rounded min-h-65 max-h-70 h-full w-full cursor-pointer'
              ...
              <div className='pt-2 flex items-center justify-center gap-3 '>
                {data.image.map((img, i) => (
                  <div
                    key={i}
                    className={`rounded-full w-3 h-3 md:w-5 md:h-5 ${i === image ? "bg-slate-300 " : "bg-slate-200"}`}
                  />
                ))}
              </div>
              ...
              <div className='grid relative'>
                <div className='flex gap-4 md:items-center md:justify-center py-3 md:py-3 overflow-x-auto scrollbar-hidden md:mt-0 p-2 md:p-0'>
                  {data.image.map((img, i) => (
                    <div
                      key={i}
                      className='md:h-30 md:w-30 lg:h-20 lg:w-20 h-20 min-h-20 min-w-20 cursor-pointer w-20 shadow'
                      onClick={() => setImage(i)}
                    >
                      <img
                        src={img}
            ...
            <div className='col-span-1 p-4 px-0 pt-3 lg:pt-4 lg:pl-6 text-base md:text-lg'>
              ...
              <div className="flex">
                ...
                {data.discount >= 1 && (
                  <div className='text-green-600 font-bold md:text-2xl text-xl p-2'>
                    {data.discount}% <span className='text-base'>Discount</span>
                  </div>
                )}
              </div>
              ...
              <div>
                <div className='flex items-center gap-4 my-4'>
                  <img src={img1} alt={'Superfast_delivery'} className='min-w-20 max-w-20 max-h-20 min-h-20' />
                  <div>
                ...
                <div className='flex items-center gap-4 my-4'>
                  <img src={img2} alt={'Best Price Offer'} className='min-w-20 max-w-20 max-h-20 min-h-20' />
                  <div>
                ...
                <div className='flex items-center gap-4 my-4'>
                  <img src={img3} alt={'Wide Assortment'} className='min-w-20 max-w-20 max-h-20 min-h-20' />
                  <div>
                    <div className='text-sm'>Wide assortment</div>
                    <p className='font-semibold'>Choose from 5000+ product across whole personal care, household & other category </p>
                  </div>
                </div>
              </div>
            </div>
      ...
      {/* ✅ Show fullscreen image when clicked */}
      {showImage && (
        <ViewImage 
